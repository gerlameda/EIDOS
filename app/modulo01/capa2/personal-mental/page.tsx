...
const AREA_ID = "personal-mental";

const P1_OPTIONS = [
  "Nunca lo he hecho de forma consciente",
  "Lo he pensado de vez en cuando pero sin profundidad",
  "Tengo momentos de claridad pero se van rápido",
  "Sí, tengo bastante claro lo que quiero",
] as const;

const P2_LABELS: [string, string, string, string, string] = [
  "Me paraliza constantemente",
  "Me frena más de lo que impulsa",
  "No la tomo en cuenta",
  "Me impulsa más de lo que frena",
  "Es mi mayor aliado",
];
...
function mensajeResultado(score: number): string {
  ...
  if (rango === "bajo") {
    return `Con ${s}% en tu mundo mental, hay más ruido que señal. Eso no es un defecto — es el punto de partida de casi todos los que terminan construyendo algo real.`;
  }
  if (rango === "medio") {
    return `${s}% en tu mente. Hay momentos de claridad y momentos de niebla. Estás en transición — lo que falta no es información, es consistencia.`;
  }
  return `${s}% en tu mundo interno. Tienes brújula. El trabajo ahora es mantenerla calibrada cuando el entorno intenta desviarte.`;
}
...
export default function Capa2PersonalMentalPage() {
    ...
    <main className="flex min-h-screen flex-col bg-[#0D0D14] text-[#F0EDE8]">
      <div className="flex flex-1 flex-col">
        ...
        {paso === 1 ? (
          <div className="flex-1 pb-6">
            <MultipleChoiceQuestion
              question="¿Alguna vez te has sentado a pensar con honestidad qué quieres realmente de tu vida?"
              options={[...P1_OPTIONS]}
              selected={p1}
              onSelect={setP1}
            />
        ...
        {paso === 2 ? (
          <div className="flex-1 pb-6">
            <Scale1to5Question
              question="¿Tu diálogo interno te impulsa o te frena?"
              labels={P2_LABELS}
              selected={p2}
              onSelect={setP2}
            />
