...
const AREA_ID = "fisica-salud";

const P1_LABELS: [string, string, string, string, string] = [
  "Siempre estoy agotado",
  "Me falta energía la mayoría del tiempo",
  "Tengo energía suficiente pero no sobra",
  "Tengo buena energía en general",
  "Me siento con energía para todo lo que quiero hacer",
];

const P2_OPTIONS = [
  "No cuido ninguno de los tres",
  "Cuido uno o dos pero el resto está descuidado",
  "Los tres están en proceso pero no son consistentes",
  "Los tres están bastante bien integrados en mi vida",
] as const;
...
function mensajeResultado(score: number): string {
  ...
  if (rango === "bajo") {
    return `${s}% en salud. Tu cuerpo está pidiendo atención básica antes de cualquier otra cosa. Sin energía física, todo lo demás cuesta el doble.`;
  }
  if (rango === "medio") {
    return `${s}% en tu cuerpo. Hay intención pero no consistencia. El sistema te va a ayudar a que lo que ya sabes que funciona, funcione de verdad.`;
  }
  return `${s}% en salud. Tu cuerpo trabaja contigo. Eso es ventaja — úsala para construir en las áreas que más lo necesitan.`;
}
...
export default function Capa2FisicaSaludPage() {
      ...
      <div className="flex flex-1 flex-col">
        ...
        {paso === 1 ? (
          <div className="flex-1 pb-6">
            <Scale1to5Question
              question="¿Tu cuerpo tiene la energía que necesitas para vivir como quieres?"
              labels={P1_LABELS}
              selected={p1}
              onSelect={setP1}
            />
        ...
        {paso === 2 ? (
          <div className="flex-1 pb-6">
            <MultipleChoiceQuestion
              question="¿Duermes bien, te mueves con consistencia y comes con intención?"
              options={[...P2_OPTIONS]}
              selected={p2}
              onSelect={setP2}
            />
