...
const AREA_ID = "financiera";

const P1_OPTIONS = [
  "No tengo idea, el dinero simplemente desaparece",
  "Tengo una idea general pero no llevo registro",
  "Lo reviso de vez en cuando pero no es consistente",
  "Sí, tengo claridad total de mis finanzas",
] as const;

const P3_LABELS: [string, string, string, string, string] = [
  "Es una fuente constante de angustia",
  "Me preocupa más de lo que me gustaría",
  "No pienso mucho en ello",
  "Lo manejo con relativa tranquilidad",
  "Tengo una relación sana y tranquila con el dinero",
];
...
function mensajeResultado(score: number): string {
  ...
  if (rango === "bajo") {
    return `${s}% en finanzas. Sin mapa financiero, el dinero desaparece antes de que puedas decidir a dónde va. El primer paso no es ganar más — es ver con claridad.`;
  }
  if (rango === "medio") {
    return `${s}%. Tienes control básico pero no estrategia. Estás administrando — todavía no estás construyendo.`;
  }
  return `${s}% en finanzas. Tienes los fundamentos. La siguiente capa es que tu dinero trabaje con la misma intención que tú.`;
}
...
export default function Capa2FinancieraPage() {
    ...
    <main className="flex min-h-screen flex-col bg-[#0D0D14] text-[#F0EDE8]">
      <div className="flex flex-1 flex-col">
        ...
        {paso === 1 ? (
          <div className="flex-1 pb-6">
            <MultipleChoiceQuestion
              question="¿Sabes exactamente cuánto entra, cuánto sale y a dónde va tu dinero?"
              options={[...P1_OPTIONS]}
              selected={p1}
              onSelect={setP1}
            />
        ...
        {paso === 2 ? (
          <div className="flex-1 pb-6">
            <SliderQuestion
              question="¿Cómo describes tu relación con el dinero hoy?"
              leftLabel="Gasto todo lo que tengo"
              rightLabel="Ahorro e invierto consistentemente"
              value={p2}
              onChange={setP2}
            />
          </div>
        ...
        {paso === 3 ? (
          <div className="flex-1 pb-6">
            <Scale1to5Question
              question="¿El dinero te genera ansiedad o te da tranquilidad?"
              labels={P3_LABELS}
              selected={p3}
              onSelect={setP3}
            />
          ...
          {paso < 5 ? (
            <button
              ...
              className={
                ...
                (!canAdvance ? "cursor-not-allowed opacity-40" : "opacity-100")
              }
            >
              {ctaLabel}
            </button>
